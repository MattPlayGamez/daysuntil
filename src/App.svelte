<script>
  ...
  let customVacations = JSON.parse(
    localStorage.getItem("customVacations") || "[]"
  );

  function buildVacationList() {
    return [...vacationData, ...customVacations]
      .map((v) => ({
        ...v,
  ...
  <section class="cards">
    ...
    <div class="card end-year">
      ...
      <div class="progress">
        <div
          class="bar"
          style="--progress:{manager.endOfSchoolYear().procent || 0}%"
        ></div>
      </div>
      ...
      {#each vacationList as v}
        <li class={v.custom ? "custom" : ""}>
          ...
          {#if v.custom}
            <button
              class="delete-btn"
              on:click={() => deleteCustomVacation(v.id)}>Delete</button
            >
          {/if}
        </li>
      {/each}
